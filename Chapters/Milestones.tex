...
%<*mlst:04i>
\begin{kaobox}[backgroundcolor=\MScolor,frametitlebackgroundcolor=\MScolor,frametitle=\showmscntr Milestone: Collection of \texttt{EC} calibration dataset]
	\begin{itemize}
		\item [$\Box$] This Milestone is to use your \texttt{EC} circuit to collect a simple calibration dataset. \,
		\begin{itemize}
			\item[$\circ$] After waterproofing your device and ensuring that it collects data, immerse the electrodes in tapwater and take a reading.  Write down the temperature and average apparent resistance values.
			\item[$\circ$] Using a separate commercial \texttt{EC} probe, measure the \texttt{EC} of the same solution and write it down. \,
			\item[$\circ$] Repeat two more times so that you have three measurements for this solution. \,
			\item[$\circ$] Now mix up a solution of water and KCl salt (or use table salt if KCl is not available) and repeat all the above steps three times.  The resistance should be lower than before. \,
			\item[$\circ$] Dilute your KCl solution by a factor of two by dumping out half and replacing with tapwater.  Repeat all the above steps to get three readings for this solution. \,
			\item[$\circ$] Dilute and repeat once more. You should end up with at least four separate sets of \texttt{EC} and average apparent resistance readings.
		\end{itemize}
%\smallskip
		When you have completed your measurements, enter your data into a spreadsheet and save as a csv file that includes columns for observed \texttt{EC}, average apparent resistance, and temperature.  Be prepared to use this in class next week.
	\end{itemize}
\end{kaobox}
%</mlst:04i>

%<*mlst:04j>
\begin{kaobox}[backgroundcolor=\MScolor,frametitlebackgroundcolor=\MScolor,frametitle=\showmscntr Milestone: Calibration of \texttt{EC} sensor]
	\begin{itemize}
		\item [$\Box$] This Milestone is to develop a predictive equation that you can use to estimate \texttt{EC} using your sensor. \,
		\begin{itemize}
			\item[$\circ$] Open the \htmladdnormallink{\texttt{Calibration Examples}}{https://github.com/jwlauer/EnvironmentalSensing/tree/master/AnalysisCode/Calibration} Jupyter Notebook and load your data spreadsheet.
			\item[$\circ$] Compute the mean value of your cell constant $\overline{k}$ using simple averaging. \,
			\item[$\circ$] Use the linear-regression based approach to derive a linear equation that gives you \texttt{EC} from a sensor reading. \,
			\item[$\circ$] Use the log-log regression based approach to derive a power function that gives you \texttt{EC} from a sensor reading. \,
			\item[$\circ$] Estimate the upper and lower 95\% confidence limits on \texttt{EC} at a typical \texttt{EC} value for your dataset based a measurement made using the log-log equation. \,
			\item[$\circ$] Recompute the limits you would get at this \texttt{EC} from the log-log equation if your post-calibration measurement protocol required that you always take four measurements of $\frac{1}{R}$ and average them before applying the your power-function based calibration equation.
		\end{itemize}
%\smallskip
		Submit your calibration curves, the associated calibration equations, and a one-paragraph description that summarizes the quality of the calibration and the likely precision of a measurement you might make using your sensor.
	\end{itemize}
\end{kaobox}
%</mlst:04j>
